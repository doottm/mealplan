...
feedbacks는 구체적이고 실용적인 한국어 조언을 3-5개 제공하세요.`;
  ...
  try {
    // 최신 안정 모델 사용 (gemini-2.5-flash-lite)
    ...
    if (!response.ok) {
      ...
      console.error(`[Gemini] API 오류 (model: ${modelName}):`, response.status, errBody);
      // 상세 진단 정보 출력
      console.error('[Gemini] API Key prefix:', apiKey.substring(0, 8) + '...');
      return res.status(502).json({ error: 'Gemini API error', detail: response.status, model: modelName });
    }
